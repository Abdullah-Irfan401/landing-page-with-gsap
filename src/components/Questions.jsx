    ...
    const toggleExpand = () => {
      ...
      return (
        ...
        <div className="flex justify-start items-start cursor-pointer" onClick={toggleExpand}>
          <div className="flex-1 pl-[5rem] text-[1.252rem] font-medium text-[#000000] ml-[-5rem]  font-pp"> {title} </div>
          <div className="text-4xl text-[#000000] "> {isExpanded ? '-' : '+'} </div>
        </div>
      
        {isExpanded && (
          <div className="mt-6  text-[#949690] font-pp w-[44rem] ">
            <p className=" text-[1.252rem] font-medium leading-[1.69rem] mb-[3rem]  ">
              {content}
            </p>
          </div>
        )}
